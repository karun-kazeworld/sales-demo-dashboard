...
import { getTotalScore, getStatus } from '../../utils/analysisHelpers';
  ...
  <>
    ...
    >
      ...
      <span style={{
        ...
        fontSize: '11px',
        fontWeight: 'bold',
        backgroundColor: getStatusColor(getStatus(conversation)),
        color: 'white',
        textAlign: 'center'
      }}>
        {getStatus(conversation).toUpperCase()}
      </span>
...
export function SupervisorDashboard() {
  ...
  const analytics: Analytics = useMemo(() => {
    ...
    // Filter conversations by supervisor's domain
    const domainConversations = conversations.filter(conv => {
      const product = productMap.get(conv.product_id);
      return product?.domain === profile?.domain || profile?.role === 'admin';
    });
  ...
  }, [conversations, products, profile?.domain, profile?.role, selectedProductId, selectedExecutiveId]);

  // Separate filtering for conversations list (includes date filter)
  const filteredConversationsForList = useMemo(() => {
    ...
    return domainConversations.filter(conv => {
      if (selectedProductId && conv.product_id !== selectedProductId) return false;
      if (selectedExecutiveId && conv.executive_id !== selectedExecutiveId) return false;
  ...
  }, [conversations, products, profile?.domain, profile?.role, selectedProductId, selectedExecutiveId, selectedDateRange, customStartDate, customEndDate]);
          ...
          <table style={{
            ...
            <thead>
              <tr>
                ...
                <th style={{ minWidth: '160px', textAlign: 'center', padding: '8px' }}>Status</th>
              </tr>
            </thead>
            <tbody>
